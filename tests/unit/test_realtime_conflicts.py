...
class TestEnemyCollision:
    ...
    def test_later_arrival_captures_earlier_arrival_on_same_square(self):
        ...
        assert state.board.get_piece_at(Cell(0, 2)) == "bR"
...
class TestFriendlyPieceLanding:
    ...
    def test_second_friendly_arrival_on_occupied_square_is_rejected(self):
        ...
        assert state.board.get_piece_at(Cell(2, 2)) == "wR"
    ...
    def test_only_the_winning_move_fires_a_completed_event(self):
        ...
        assert obs.completed[0].from_pos == Cell(0, 0)
    ...
    def test_blocked_piece_is_no_longer_in_transit_and_can_be_reselected(self):
        ...
        engine.handle_click(state, 200, 0)
        engine.tick(state, 2000)
        assert engine.is_in_transit(state, Cell(2, 2)) is False
        engine.handle_click(state, 200, 200)
        assert engine.selection == Cell(2, 2)
...
class TestInvalidPremoves:
    def test_sliding_move_blocked_by_piece_that_moved_into_its_path(self):
        ...
        assert state.board.get_piece_at(Cell(0, 1)) == "bK"
        assert state.board.get_piece_at(Cell(0, 3)) == "."
    ...
    def test_origin_piece_captured_before_its_own_move_can_execute(self):
        ...
        assert state.board.get_piece_at(Cell(0, 0)) == "bR"
        assert state.board.get_piece_at(Cell(0, 2)) == "."
...
class TestFriendlyMidRouteBlock:
    def test_rook_stops_one_square_before_a_friendly_blocker(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 1)) == "wR"  # stopped short
        assert state.board.get_piece_at(Cell(0, 2)) == "wR"  # the blocker
        assert state.board.get_piece_at(Cell(0, 0)) == "."   # origin vacated
        assert state.board.get_piece_at(Cell(0, 3)) == "."   # never reached
    ...
    def test_bishop_stops_before_a_friendly_blocker_on_the_diagonal(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 0)) == "wB"  # never left origin
        assert state.board.get_piece_at(Cell(1, 1)) == "wN"  # the blocker
        assert state.board.get_piece_at(Cell(3, 3)) == "."   # never reached
    ...
    def test_stopped_piece_fires_move_completed_event_with_the_short_destination(self):
        ...
        engine.tick(state, 3000)
        completed = [c for c in obs.completed if c.piece == "wR" and c.from_pos == Cell(0, 0)]
        assert len(completed) == 1
        assert completed[0].to_pos == Cell(0, 1)
    ...
    def test_blocker_immediately_adjacent_to_origin_means_no_movement_at_all(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 0)) == "wR"  # never moved
        assert not any(c.from_pos == Cell(0, 0) for c in obs.completed)
    ...
    def test_enemy_blocker_mid_route_still_drops_the_whole_move(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 0)) == "wR"  # dropped, not stopped short
    ...
    def test_knight_move_is_never_affected_by_this_rule(self):
        ...
        engine.tick(state, 2000)
        assert state.board.get_piece_at(Cell(2, 1)) == "wN"
    ...
    def test_pawn_two_step_blocked_by_friendly_on_its_only_intermediate_square(self):
        ...
        engine.tick(state, 2000)
        assert state.board.get_piece_at(Cell(2, 0)) == "wP"  # never advanced
        assert state.board.get_piece_at(Cell(1, 0)) == "wN"  # blocker unmoved
        assert state.board.get_piece_at(Cell(0, 0)) == "."
    ...
    def test_second_rook_stops_behind_wherever_the_first_one_lands(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 2)) == "wR"  # Rook A's new spot
        assert state.board.get_piece_at(Cell(0, 3)) == "wR"  # Rook B stopped short
        assert state.board.get_piece_at(Cell(0, 0)) == "."   # Rook A's old spot
        assert state.board.get_piece_at(Cell(0, 4)) == "."   # Rook B's old spot
...
class TestCheckpointTimingGranularity:
    def test_mid_path_block_resolves_at_its_own_checkpoint_not_at_the_final_arrival_time(self):
        ...
        assert state.board.get_piece_at(Cell(0, 1)) == "wR"  # already stopped short
        assert state.board.get_piece_at(Cell(0, 2)) == "wR"  # the blocker
        assert state.board.get_piece_at(Cell(0, 3)) == "."   # never reached
    ...
    def test_a_blocker_gone_by_the_old_arrival_time_still_stops_the_move(self):
        ...
        assert state.board.get_piece_at(Cell(0, 1)) == "wR"  # first rook, stopped short
        assert state.board.get_piece_at(Cell(0, 2)) == "."   # blocker already left
        assert state.board.get_piece_at(Cell(0, 3)) == "."   # never reached
        assert state.board.get_piece_at(Cell(1, 2)) == "wR"  # blocker's new position
...
class TestChronologicalDueOrder:
    ...
    def test_both_moves_land_correctly_despite_reversed_queue_order(self):
        ...
        engine.tick(state, 3000)
        assert state.board.get_piece_at(Cell(0, 3)) == "wR"
        assert state.board.get_piece_at(Cell(1, 3)) == "bK"
...
class TestLaneHelper:
    def test_horizontal_move_returns_col_lane(self):
        assert GameEngine._lane(Cell(0, 0), Cell(0, 2)) == ("col", 0, 2)

    def test_horizontal_move_lane_is_order_independent(self):
        assert GameEngine._lane(Cell(0, 2), Cell(0, 0)) == ("col", 0, 2)

    def test_vertical_move_returns_row_lane(self):
        assert GameEngine._lane(Cell(0, 1), Cell(3, 1)) == ("row", 0, 3)

    def test_diagonal_move_returns_none(self):
        assert GameEngine._lane(Cell(0, 0), Cell(2, 2)) is None

    def test_knight_shaped_move_returns_none(self):
        assert GameEngine._lane(Cell(0, 0), Cell(2, 1)) is None

    def test_zero_distance_returns_none(self):
        assert GameEngine._lane(Cell(1, 1), Cell(1, 1)) is None
