...
def _pm(piece: str, from_pos: Cell, to_pos: Cell) -> PendingMove:
...
class TestAirborneDefender:
    ...
    def test_returns_none_when_nothing_is_airborne_at_the_cell(self):
        assert self.resolver.airborne_defender(Cell(1, 0), "bR", []) is None

    def test_returns_none_when_the_airborne_piece_is_friendly(self):
        airborne = [PendingJump(piece="wK", pos=Cell(1, 0), land_time=1000)]
        assert self.resolver.airborne_defender(Cell(1, 0), "wR", airborne) is None

    def test_returns_the_defender_when_it_is_an_enemy(self):
        airborne = [PendingJump(piece="wK", pos=Cell(1, 0), land_time=1000)]
        defender = self.resolver.airborne_defender(Cell(1, 0), "bR", airborne)
        assert defender is not None
        assert defender.piece == "wK"

    def test_ignores_airborne_pieces_at_other_cells(self):
        airborne = [PendingJump(piece="wK", pos=Cell(2, 2), land_time=1000)]
        assert self.resolver.airborne_defender(Cell(1, 0), "bR", airborne) is None
