...
class TestNewGame:
    ...
    def test_builds_the_standard_board_when_no_board_arg_was_given(self):
        engine, state = main_gui._new_game(self._args(), BoardMapper(100))
        assert isinstance(engine, GameEngine)
        assert isinstance(state, GameState)
        assert state.board.get_rows() == main_gui.STANDARD_BOARD_ROWS

    def test_uses_the_same_board_file_a_custom_launch_used(self, tmp_path):
        board_file = tmp_path / "custom.txt"
        board_file.write_text("wK . .\n. . .\n. . bK\n")

        _, state = main_gui._new_game(self._args(board_file), BoardMapper(100))
    ...
    def test_two_calls_return_independent_engines_and_states(self):
        engine1, state1 = main_gui._new_game(self._args(), BoardMapper(100))
        engine2, state2 = main_gui._new_game(self._args(), BoardMapper(100))
    ...
    def test_a_fresh_engine_correctly_detects_the_new_games_own_king_loss(self):
        ...
        fresh_engine, fresh_state = main_gui._new_game(self._args(), BoardMapper(100))
